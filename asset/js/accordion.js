const accordionScript = () => {
    ...
    for (let i = 0; i < accordionTrigger.length; i++) {
        ...
        if (i == 0) {
            const firstAccordion = accordionTrigger[i];
            firstAccordion.classList.toggle('expanded');
            firstAccordion.setAttribute('aria-expanded', firstAccordion.classList.contains('expanded'));
            firstAccordion.parentElement.parentElement.classList.toggle('expanded');
            const panel = firstAccordion.parentElement.nextElementSibling;
            if (panel.style.maxHeight) {
                panel.style.maxHeight = null;
            } else {
                panel.style.maxHeight = panel.scrollHeight + 'px';
